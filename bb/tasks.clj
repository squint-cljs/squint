...
(defn test-project [_]
  (let [dir "test-project"]
    ...
    (let [output (:out (shell {:dir dir :out :string} "node lib/main.mjs"))]
      ...
      (assert (str/includes? output "no-alias qualified: Hello, world!"))
      (assert (str/includes? output "real-debug:"))
      (assert (str/includes? output "macro-dep: [test] 999"))
      (assert (str/includes? output "deep-dep: [deep] 777")))
