...
- Fix [#832](https://github.com/squint-cljs/squint/issues/832): nREPL server hung on advertised ops whose handlers were stubs that never replied. The ops are now implemented off the compiler ns-state: `info`/`lookup` and `eldoc` return arglists/doc for session-defined vars (or `no-info`/`no-eldoc` when unknown), `complete` returns candidates from the current ns (locals, refers, aliases) plus JS-interop names for a `js/...` prefix (e.g. `js/console.lo` -> `js/console.log`, enumerated from the browser's globals when eval is delegated there, else node's), and `load-file` evaluates the file contents. To support this, `defn`/`def` now record `:arglists` and `:doc` in the var metadata held by ns-state.
