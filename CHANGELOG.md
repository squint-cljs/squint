...
- Optimize compiled output for `or`, `and` and `reify`
- Fix [#1000](https://github.com/squint-cljs/squint/issues/1000): treat empty strings from `str`, `subs` or `name` as truthy in `if`
- Allow locals to shadow core macros in call position
- Fix map destructuring with a symbol key
- Fix `reduce-kv` to stop on `reduced` and pass vector indices as keys
- Fix `with-meta` to preserve protocol methods on `deftype`, `defrecord` and `reify` instances
- Fix `recur` in protocol methods
- Fix [#994](https://github.com/squint-cljs/squint/issues/994): JSX fragments with `:jsx-runtime`
- Fix compilation of `set!`, `js-in` and `js/typeof` in expressions ([#1011](https://github.com/squint-cljs/squint/issues/1011), [#1002](https://github.com/squint-cljs/squint/issues/1002))
- Fix `eval` as a var, local or parameter name
- Fix [#1008](https://github.com/squint-cljs/squint/issues/1008): `and` with `recur` after a boolean operand
- Resolve qualified and dotted var names in the current or a required namespace
- Fix regex literals on the JVM compiler path, including return values ([#992](https://github.com/squint-cljs/squint/issues/992))
- Fix [#998](https://github.com/squint-cljs/squint/issues/998): `cljs.test` macros with a custom `:resolve-ns`
- Preserve the type of `deftype` instances in `assoc`
- Fix `map?`, `object?` and printing for `reify` instances
- Preserve form metadata on `reify`
- Make `satisfies?` and `implements?` consistently return booleans
- Fix `deftype` with `Object` methods adding an `undefined` property
- Playground: fix results after toggling REPL mode and stale errors and compiler state between runs ([#990](https://github.com/squint-cljs/squint/issues/990))
- Playground: update layout, controls, panel scrolling and result font size ([#989](https://github.com/squint-cljs/squint/pull/989), [@paintparty](https://github.com/paintparty))
...
- Fix [#975](https://github.com/squint-cljs/squint/issues/975): `& {:keys [...]}` now destructures a map instead of the raw rest args, and a seq destructured as a map is read as kwargs. Both the kwargs and trailing-map call styles work, via `seq-to-map-for-destructuring`.
