...
(deftest ^:async prewalk-order-test
...
(deftest ^:async postwalk-order-test
  (evalll (eq [1 2
               :a 3 [:a 3] {:a 3}
               4 5 [5] (list 4 [5])
               [1 2 {:a 3} (list 4 [5])]])
...
(deftest ^:async walk-list-test
  (evalll (eq 9)
          '(do (ns foo (:require [clojure.walk :as w]))
               (def result (w/walk inc #(reduce + %) (list 1 2 3))))))

(deftest ^:async walk-vector-test
  (evalll (eq 9)
          '(do (ns foo (:require [clojure.walk :as w]))
               (def result (w/walk inc #(reduce + %) [1 2 3])))))

(deftest ^:async walk-set-test
  (evalll (eq 9)
          '(do (ns foo (:require [clojure.walk :as w]))
               (def result (w/walk inc #(reduce + %) #{1 2 3})))))

(deftest ^:async walk-map-test
  (evalll (eq 9)
          '(do (ns foo (:require [clojure.walk :as w]))
...
(deftest ^:async walk-preserves-collection-type-test
  (evalll (eq true)
          '(do (ns foo (:require [clojure.walk :as w]))
...
(deftest ^:async stringify-keys-test
  (evalll (eq {:a 1, :b {:c 2}})
          '(do (ns foo (:require [clojure.walk :as w]))
               (def result (w/stringify-keys {:a 1 :b {:c 2}})))))

(deftest ^:async keywordize-keys-test
  (evalll (eq {:a 1, :b {:c 2}})
          '(do (ns foo (:require [clojure.walk :as w]))
...
(deftest ^:async postwalk-preserves-meta-test
  (evalll (eq {:foo 3})
          '(do (ns foo (:require [clojure.walk :as w]))
...
(deftest ^:async prewalk-preserves-meta-test
  (evalll (eq {:foo 3})
          '(do (ns foo (:require [clojure.walk :as w]))
...
(deftest ^:async walk-retains-meta-on-root-test
