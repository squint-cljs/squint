...
(defn game->ui-data [{:keys [size tics victory over?]}]
  ;; squint: sets use reference equality, so a set of [y x] vectors can't be
  ;; queried by value. Compare against the path with `=` instead.
  (let [highlight? (fn [yx] (some #(= yx %) (:path victory)))]
    {:button (when over?
               {:text "Start over"
                :on-click [:reset]})
     :board
        ...
        (for [x (range size)]
          (if-let [player (get tics [y x])]
            (let [victorious? (highlight? [y x])]
              (cond-> {:content (get player->mark player)}
