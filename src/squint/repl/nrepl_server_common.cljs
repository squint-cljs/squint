(ns squint.repl.nrepl-server-common
  "nREPL server shared between squint and cherry (see cherry ADR 0004).
  Dialect-specific bits come in via set-dialect!:

  {:compile-string* compiler/compile-string* ;; (fn [code opts prev-state])
   :resolve-ns-repl compiler-node/resolve-ns-repl
   :config-file     \"squint.edn\"
   :pr-str-repl     print/pr-str-repl}       ;; (fn [v]) -> Promise<string>"
  (:require
  ...
  (if nrepl-pprint
    (if-let [pprint-fn (pretty-print-fns-map nrepl-pprint)]
      ...
      (do
        ...
        ((:pr-str-repl @!dialect) value)))
    ;; the dialect's pr-str-repl: pr-str + Promise-aware rendering so an
...
(defn compile [the-val ns]
  (let [;; Apply the config file's :jsx-runtime so #jsx eval'd at the REPL
        ...
        jsx-runtime (some-> (utils/get-cfg (:config-file @!dialect))
                            :jsx-runtime (assoc :development true))
...
(defn handle-lookup [{:keys [op ns] :as request} send-fn]
  ;; Resolve a var's metadata from the compiler ns-state (no runtime ns
  ;; environment; arglists/doc are captured at compile time, keyed by the
