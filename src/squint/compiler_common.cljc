  ...
  (let [gensym (:gensym env)
        ...
        ret (cond-> (str
                     ...
                     (str/join (map (fn [test then]
                                      (str/join
                                       (map (fn [test]
                                              (str ;; a keyword test also matches its
                                                   ;; name string (name-equivalent =)
...
(defmethod emit-special 'funcall [_type env [fname & args :as expr]]
  (let [ns (when (symbol? fname) (namespace fname))
        ...
        ;; 'string stays callable-as-lookup: a var bound to (name k) etc.
        ;; keeps the old keywords-are-strings call behavior
             ...
             (str/join " "
                       (map
                        (fn [[k v]]
                          ...
                          ;; like the old keywords-are-strings emission
