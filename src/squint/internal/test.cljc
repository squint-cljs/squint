...
;; Extensible like cljs.test/assert-expr: dispatch on the head symbol of the
;; asserted form. The same multimethod object is exposed to the macro-loader
;; (SCI) under cljs.test/clojure.test/squint.test, so a compile-time user
;; (defmethod cljs.test/assert-expr 'my/op [menv msg form] ...) registers on it
;; and this compiler-side dispatch sees it.
